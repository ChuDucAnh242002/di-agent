"""Modbus TCP interface mirroring the real-world link between a genset
engine controller (e.g. DEIF, ComAp) and a vessel's power management system:
the PMS reads status/power over input registers and writes the load setpoint
over a holding register, instead of (or alongside) a REST/Kafka pipeline.
...
class GensetModbusServer:
    """Runs a Modbus TCP server backed by the given GensetController, in
